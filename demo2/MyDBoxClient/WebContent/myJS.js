...
function doQuery1_back(result)
{
  	...
  	var accessTokenField = document.querySelector('#accessTokenField');
  	var accountTokenField = document.querySelector('#accountTokenField');
  	accessTokenField.value = accessToken;
  	accountTokenField.value = account_id;
...
//	window.document.getElementById('div5').style.visibility = 'hidden';
    ...
    // Add an event listener to the file input to handle the selected file
        fileInput.addEventListener('change', function() {
            ...
            if (selectedFile) {
                fileDetails.textContent = `Selected File: ${selectedFile.name}, Size: ${selectedFile.size} bytes`;
            } else {
